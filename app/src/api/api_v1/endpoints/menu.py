...
@router.get("/", response_model=MenuRead, status_code=status.HTTP_200_OK)
...
def create_menu(item_in: MenuCreate, db: Session = Depends(get_db)):
    response = crud.menu.create(db=db, obj_in=item_in)
    return response


@router.patch("/{menu_id}", response_model=MenuRead)
def update_menu(menu_id: int, item_in: MenuUpdate, db: Session = Depends(get_db)):
