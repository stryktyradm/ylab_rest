...
from src.api.deps import get_db, get_menu_model
...
def get_submenus(db: Session = Depends(get_db), menu_model: Menu = Depends(get_menu_model)):
    response = crud.submenu.get_list(db=db, menu_id=menu_model.id)
    return response
...
def update_submenu(submenu_id: int, item_id: SubMenuUpdate, db: Session = Depends(get_db)):
