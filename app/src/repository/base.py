...
class BaseCRUDRepository(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def get_list(self, **kwargs: dict) -> Sequence[ModelType]:
        stmt = select(self.model).filter_by(**kwargs)
        result = await self.db_session.execute(stmt)
        rows = result.scalars().all()
        return rows

    async def create(self, obj_in: CreateSchemaType, **kwargs: dict) -> ModelType:
    ...
    async def update(self, id_: str, obj_in: UpdateSchemaType) -> ModelType:
        db_obj = await self.get(id_=id_)
